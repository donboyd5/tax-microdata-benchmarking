...
get_colmap <- function(tab, DATADIR, targfn) {
  ...
  col_map <- readxl::read_excel(
    ...
    select(all_of(c(stub_vars, year_vars))) |>
    pivot_longer(
      -all_of(stub_vars),
      names_to = "year",
      values_to = "xlcolumn"
    ...
    select(table, year, all_of(stub_vars), xl_colnumber, xlcolumn) |>
...
ptargets_for_checking <- table_data |>
  ...
  mutate(
    ptarget = as.numeric(ptarget),
    ptarget = ifelse(var_type == "amount", ptarget * 1000, ptarget)
  ) |> # every column had better be a number stored as text! ALSO, SCALE to dollars
  select(
    year,
    all_of(stub_vars),
    table,
