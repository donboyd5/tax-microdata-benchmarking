...
def _build_constraint_matrix(area, vardf, out):
    """
...
def _drop_impossible_targets(B_csc, targets, labels, out):
    ...
    if all_zero.any():
        ...
        keep = ~all_zero
        # reconstruct B_csc keeping only non-zero rows
...
def create_area_weights_file_clarabel(
    ...
    out.write(f"Constraint tolerance:" f" +/-{constraint_tol * 100:.1f}%\n")
    out.write(f"Multiplier bounds:" f" [{multiplier_min}, {multiplier_max}]\n")
...
if __name__ == "__main__":
    ...
    if not target_file.exists():
        ...
        sys.exit(1)
    RCODE = create_area_weights_file_clarabel(
        area_code,
        write_log=False,
        write_file=True,
    )
